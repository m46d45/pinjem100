...
const hasOgSkill = existsSync(join(TEMPLATE_ROOT, ".grok/skills/og/SKILL.md"));
const hasAgents = existsSync(join(TEMPLATE_ROOT, "AGENTS.md"));
...
test("SKILL.md and AGENTS.md name the marker path and bound this script uses", (t) => {
  if (!hasOgSkill || !hasAgents) {
    t.skip("platform .grok/skills/og + AGENTS.md not shipped in this workspace");
    return;
  }
...
test("SKILL.md tells the pass to self-check with the flag this CLI accepts", (t) => {
  if (!hasOgSkill) {
    t.skip("platform .grok/skills/og not shipped in this workspace");
    return;
  }
